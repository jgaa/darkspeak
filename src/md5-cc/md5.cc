...
/** 2016-06-12 jgaa: Modified to use modern C++.
 *                   Also removed some memory leaks.
 */
...
std::unique_ptr<std::array<unsigned char, 16>>
MD5::raw_digest(){

  auto s = std::make_unique<std::array<unsigned char, 16>>();

  if (!finalized){
      assert(false && "Can't get digest if you haven't finalized the digest!");
      throw std::runtime_error("Can't get digest if you haven't finalized the digest!");
  }
...
std::unique_ptr<std::array<char, 33>>
MD5::hex_digest(){

  int i = 0;
  auto s = std::make_unique<std::array<char, 33>>();
  ...
  for (i=0; i<16; i++)
    sprintf(s->data()+i*2, "%02x", digest[i]);
