...
void Database::createDatabase()
{
    ...
    try {
        ...
        exec(R"(CREATE TABLE "message" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `direction` INTEGER NOT NULL, `conversation` INTEGER NOT NULL, `contact` INTEGER, `message_id` TEXT, `composed_time` INTEGER NOT NULL, `sent_time` INTEGER, `received_time` INTEGER, `content` TEXT, `signature` TEXT, `from` TEXT, `encoding` INTEGER, FOREIGN KEY(`conversation`) REFERENCES `conversation`(`id`), FOREIGN KEY(`contact`) REFERENCES `contact`(`id`) ))");
        exec(R"(CREATE UNIQUE INDEX `ix_message_id` ON `message` (`contact` ,`message_id` ))");
        QSqlQuery query(db_);
