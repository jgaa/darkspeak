...
void Database::createDatabase()
{
    ...
    try {
        ...
        exec(R"(CREATE TABLE "identity" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `uuid` BLOB NOT NULL UNIQUE, `hash` BLOB NOT NULL, `name` TEXT NOT NULL, `cert` BLOB NOT NULL, `address` TEXT NOT NULL, `address_data` TEXT, `notes` TEXT, `avatar` BLOB, `created` TEXT NOT NULL ))");
        exec(R"(CREATE TABLE "contact" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `identity` INTEGER NOT NULL, `uuid` BLOB NOT NULL UNIQUE, `name` TEXT NOT NULL, `nickname` TEXT, `pubkey` BLOB NOT NULL, `address` TEXT NOT NULL, `notes` TEXT, `contact_group` TEXT NOT NULL DEFAULT 'other', `avatar` BLOB, `created` TEXT NOT NULL, `initiated_by` TEXT NOT NULL, `last_seen` TEXT, `state` INTEGER NOT NULL DEFAULT 0, `addme_message` TEXT DEFAULT 0, `autoconnect` INTEGER NOT NULL DEFAULT 0, `hash` BLOB NOT NULL, FOREIGN KEY(`identity`) REFERENCES `identity`(`id`) ))");
        ...
        exec(R"(CREATE TABLE `notification` ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `status` INTEGER NOT NULL, `priority` INTEGER NOT NULL, `identity` INTEGER NOT NULL, `contact` INTEGER, `type` INTEGER NOT NULL, `timestamp` TEXT NOT NULL, `message` TEXT, `data` BLOB ))");
