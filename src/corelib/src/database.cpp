...
void Database::createDatabase()
{
    ...
    try {
        ...
        exec(R"(CREATE TABLE "contact" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `identity` INTEGER NOT NULL, `uuid` TEXT NOT NULL UNIQUE, `name` TEXT NOT NULL, `nickname` TEXT UNIQUE, `pubkey` TEXT NOT NULL UNIQUE, `address` TEXT NOT NULL, `notes` TEXT, `contact_group` TEXT NOT NULL DEFAULT 'other', `avatar` BLOB, `created` TEXT NOT NULL, `initiated_by` TEXT NOT NULL, `last_seen` TEXT, `blocked` INTEGER NOT NULL DEFAULT 0, `rejected` INTEGER NOT NULL DEFAULT 0, `autoconnect` INTEGER NOT NULL DEFAULT 0, `hash` TEXT NOT NULL, FOREIGN KEY(`identity`) REFERENCES `identity`(`id`) ))");
